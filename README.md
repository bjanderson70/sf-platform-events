...
| 2 | **evt_PlatformEvtBuilder** builder = new evt_PlatformEvtBuilder(‘pe_test__e','test'); | Create a platform event builder. The event name, ‘*pe_test__e’,* along with the environment, ‘test’, is looked up in the custom metadata. The custom metadata may contain, four handlers. Handlers are classes that perform the following: (1) Log instrumentation information (2) Error information, that may occur (3) Log Event Data (JSON) (4) Alert of steps being performed. |
